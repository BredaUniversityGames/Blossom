...
void BeginLinkDestroy(ImNodesEditorContext& editor, const int link_idx)
{
    GImNodes->DeletedLinkIdx = link_idx;
}
...
void BeginNodeEditor()
{
    ...
    GImNodes->AltLeftMouseClicked = ImGui::IsMouseClicked(0) && ImGui::IsKeyDown(ImGui::GetKeyIndex(ImGuiKey_LeftAlt));
...
void EndNodeEditor()
{
    ...
    if (!IsMiniMapHovered())
    {
        if (GImNodes->AltLeftMouseClicked && GImNodes->HoveredLinkIdx.HasValue())
        {
            BeginLinkDestroy(editor, GImNodes->HoveredLinkIdx.Value());
        }
