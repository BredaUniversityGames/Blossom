...
void bee::ImGuiImpl::InitializeImGui()
{
    ImGui::CreateContext();
...
void bee::ImGuiImpl::SetImGuiFont(const std::string& path)
{
    ...
    io.Fonts->AddFontFromFileTTF(
        Engine.FileIO().GetPath(FileIO::Directory::Asset, "/fonts/DroidSans.ttf").c_str(),
    ...
    std::string fontpath = Engine.FileIO().GetPath(FileIO::Directory::Asset, "/fonts/FontAwesome5FreeSolid900.otf");
    io.Fonts->AddFontFromFileTTF(fontpath.c_str(), iconSize * UIScale, &config, icons_ranges);
}

void bee::ImGuiImpl::SetImGuiStyle()
{
    //ImNodes style
...
void bee::ImGuiImpl::StartFrame()
{
	ImGui_Impl_NewFrame();
	ImGuizmo::BeginFrame();
}

void bee::ImGuiImpl::EndFrame()
{
	ImGui::Render();
	ImGui_Impl_RenderDrawData(ImGui::GetDrawData());
}

void bee::ImGuiImpl::ShutdownImGui()
{
	ImGui_Impl_Shutdown();
