layout (std140, binding = CAMERA_UBO_LOCATION) uniform CameraUBO
{
    ...
    vec2 bee_resolution;              // 8
    vec4 bee_FogColor;                // 16
    float bee_FogFar;                 // 4
    float bee_FogNear;                // 4
    float bee_ambientFactor;          // 4
    int bee_cascadeCount;             // 4
    vec4 bee_cascadePlaneDistances[4]; // 16 * 4
    float bee_farPlane;               // 4
};
...
layout(std140, binding=AMBIENT_WIND_LOCATION) uniform wind_buffer
{
