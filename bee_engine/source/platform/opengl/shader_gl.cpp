...
Shader::Shader(FileIO::Directory directory, const std::string& vertexFilename, const std::string& tessControlFilename,
    const std::string& tessEvalFilename, const std::string& fragmentFilename)
    : 
      m_vertexFilename(vertexFilename),
      m_fragmentFilename(fragmentFilename),
      m_tessControlFilename(tessControlFilename),
      m_tessEvalFilename(tessEvalFilename),
      m_computeFilename("")
{

    m_directory = directory;
    m_resourcePath = vertexFilename + " " + tessControlFilename + " " + tessEvalFilename + " " + fragmentFilename;
...
void Shader::Activate() const { glUseProgram(GetProgram()); }
...
bool Shader::Load()
{
    ...
    if (m_tessControlFilename.length() > 0) tessControlSource = preprocessor.Read(m_directory, m_tessControlFilename);

    string tessEvalSource = "";
    if (m_tessEvalFilename.length() > 0) tessEvalSource = preprocessor.Read(m_directory, m_tessEvalFilename);
    ...
    return LoadSource(vertShaderSource, tessControlSource, tessEvalSource, geomShaderSource, fragShaderSource, computeShaderSource);
}
...
bool Shader::LoadSource(const string& vertexShader, const std::string& tessControlShader,
                        const std::string& tessEvalShader, const string& geometryShader, const string& fragmentShader,
                        const string& computeShader)
{
    ...
    if (tessControlShader.length() > 0)
    {
        res = CompileShader(&tessContShader, GL_TESS_CONTROL_SHADER, tessControlShader.c_str());
        if (!res)
        {
            Log::Error("Shader::Load() Failed to compile tess control shader. File: {}", m_tessControlFilename.c_str());
            return false;
        }
    }

    if (tessEvalShader.length() > 0)
    {
        res = CompileShader(&tessEvShader, GL_TESS_EVALUATION_SHADER, tessEvalShader.c_str());
        if (!res)
        {
            Log::Error("Shader::Load() Failed to compile tess evaluation shader. File: {}", m_tessEvalFilename.c_str());
            return false;
        }
    }
    ...
    if (tessContShader) glAttachShader(m_program, tessContShader);

    // Attach fragment shader to program
    if (tessEvShader) glAttachShader(m_program, tessEvShader);
...
std::string bee::Shader::GetPath(FileIO::Directory, const std::string& computeFilename) { return computeFilename; }
