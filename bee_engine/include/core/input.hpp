...
public:
    ...
    /// Checks if a gamepad has been updated more recently then keyboard and mouse
    bool IsGamepadActive() const;

    /// Checks if keyboard and mouse has been updated more recently then a gamepad
    ...
    /// Gets the screen position of the mouse, relative to the top-left corner of the viewport and normalized:
    /// Width and Height are in the ranges [-aspect ratio, aspect ration] and [-1.0, 1.0]
    glm::vec2 GetProjectedMousePosition() const;

    /// Gets the movement vector of the mouse across the screen
    ...
    void SetGameAreaPosition(float x, float y);
    void SetGameAreaSize(float width, float height);

    glm::vec2 GetGameAreaPosition() const;
    glm::vec2 GetGameAreaSize() const;
